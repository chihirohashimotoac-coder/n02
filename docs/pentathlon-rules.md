...
- **マーク表記の進み方**：ダーツライブのクリケットと同じく、未オープン（空白）→ `╱`（1マーク）→
  `✕`（2マーク）→ `⊗`（3マーク＝クローズ）。クローズ後の余剰マークはPOINTSに加算されます。
...
- **X01系のUI**：ペンタスロンの301／501は、通常01・チェックアウト練習とまったく同じ画面
  （同じヘッダー・ラウンド別スコア表・フッター・テンキー）を使います。通常01・チェックアウト練習
...
- **PCでのキーボード操作**（2026-08-28、ユーザー指示）：301/501は従来どおり数字キー・Enter・
  Backspace・U で操作できます。ダーツ入力系の種目は、画面の形に合わせて2通りを併用します。
